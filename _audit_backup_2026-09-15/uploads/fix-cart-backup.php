...
add_action('wp_head', function() {
    ...
    /* Mobile visibility fix - ensure cart toggle is visible */
    ...
    /* ===== NAV MENU FIX =====
       Keep the menu's visual styling only. Do NOT override Elementor's
       positioning/stretch logic (position, top/left/right, width) or its
       max-height controller, because those conflicting overrides pushed the
       dropdown off-screen and made it overlap the header on mobile. Elementor
       snaps the dropdown to the full viewport width on its own. */
    .elementor-71 .elementor-nav-menu--dropdown {
        background-color: #ffffff !important;
        border-radius: 0 0 12px 12px !important;
        box-shadow: 0 12px 30px rgba(0,0,0,0.18) !important;
        margin-top: 12px !important;
        border-top: 3px solid #3ABEFF !important;
    }
    .elementor-71 .elementor-nav-menu--dropdown a {
    ...
    .elementor-71 .elementor-nav-menu--dropdown a:hover,
    .elementor-71 .elementor-nav-menu--dropdown a.elementor-item-active,
    .elementor-71 .elementor-nav-menu--dropdown a:focus {
        background: #F5F5F5 !important;
        color: #10395E !important;
    }
    .elementor-71 .elementor-nav-menu--dropdown li {
        border-bottom: 1px solid #F0F0F0 !important;
    }
    .elementor-71 .elementor-nav-menu--dropdown li:last-child {
        border-bottom: none !important;
    }
    /* Mobile: keep the open menu inside the viewport (never off-screen).
       Elementor measures the badge/toggle and sets --menu-height, but we cap it
       against the actual viewport so long menus scroll instead of overflowing. */
    @media (max-width: 1024px) {
        .elementor-71 .elementor-nav-menu--toggle .elementor-menu-toggle.elementor-active
    ...
    /* Ensure hamburger icon visible on glass header */
    .elementor-71 .elementor-menu-toggle {
        ...
        function initJuhaniCartFix() {
            ...
            const tryInit = () => {
                ...
                if (menuToggle && menuDropdown) {
                    console.log('[Juhani Menu Fix] Found menu toggle');
                    // Ensure menu toggle has proper fallback if Elementor JS fails
                    if (!menuToggle.dataset.juhaniMenuFixed) {
                        menuToggle.dataset.juhaniMenuFixed = '1';
                        menuToggle.addEventListener('click', function(e) {
                            // Let Elementor handle first, then check after 200ms
                            setTimeout(() => {
                                const isActive = menuToggle.classList.contains('elementor-active');
                                // If Elementor didn't toggle, do it manually.
                                // Do NOT force a fixed max-height or overflow:visible here —
                                // doing so makes the menu spill over the header and off-screen.
                                if (!isActive) {
                                    console.log('[Juhani Menu Fix] Manual menu toggle (open)');
                                    menuToggle.classList.add('elementor-active');
                                    menuToggle.setAttribute('aria-expanded', 'true');
                                    menuDropdown.setAttribute('aria-hidden', 'false');
                                    // Cap height against the viewport so it never overflows off-screen.
                                    menuDropdown.style.maxHeight = 'calc(100vh - 90px)';
                                    menuDropdown.style.transform = 'scaleY(1)';
                                } else {
                                    console.log('[Juhani Menu Fix] Manual menu toggle (close)');
