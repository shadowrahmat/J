...
wp_register_ability('tpae/tpae-post-meta', [
    'label' => __('Post Meta', 'tpebl'),
    'description' => __('Adds the The Plus "Post Meta" widget (tp-post-meta) to an Elementor container.', 'tpebl'),
    'category' => 'tpae',
    'input_schema' => [
        'type' => 'object',
        'properties' => [
            ...
            'metaLayout' => [
                ...
                'description' => 'Meta info layout style',
            ],
            'sortfield' => [
                'type' => 'string',
                'enum' => ['date', 'category', 'author', 'comments'],
                'description' => 'Which meta fields to display',
            ],
            'date_type' => [
                'type' => 'string',
                'enum' => ['post_published', 'post_modified'],
                'description' => 'Show published date or last modified date',
            ],
    ...
    'execute_callback' => 'tpae_mcp_add_theplus_post_meta_ability',
    'permission_callback' => 'tpae_mcp_add_theplus_post_meta_permission',
    'meta' => [
        'show_in_rest' => true,
 ...
 */
function tpae_mcp_add_theplus_post_meta_permission(?array $input = null): bool
{
...
/**
 * Add a TP Post Meta widget into an Elementor container.
 *
 * @param array<string, mixed> $input
 * @return array<string, mixed>|WP_Error
 */
function tpae_mcp_add_theplus_post_meta_ability(array $input)
{
    if (!tpae_mcp_has_elementor()) {
        return new WP_Error('elementor_missing', __('Elementor must be active.', 'tpebl'));
    }

    $widget_type = 'tp-post-meta';
    if (!tpae_mcp_has_registered_widget($widget_type)) {
        return new WP_Error('widget_missing', __('The Plus Post Meta widget is not registered on this site.', 'tpebl'));
    }
    ...
    if (!empty($input['metaLayout'])) {
        $settings['metaLayout'] = sanitize_key((string) $input['metaLayout']);
    }

    if (!empty($input['sortfield'])) {
        $settings['sortfield'] = sanitize_key((string) $input['sortfield']);
    }

    if (!empty($input['date_type'])) {
        $settings['date_type'] = sanitize_key((string) $input['date_type']);
    }
