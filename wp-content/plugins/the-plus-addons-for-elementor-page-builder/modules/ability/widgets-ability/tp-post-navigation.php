...
wp_register_ability('tpae/tpae-post-navigation', [
    'label' => __('Post Navigation', 'tpebl'),
    'description' => __('Adds the The Plus "Post Navigation" widget (tp-post-navigation) to an Elementor container.', 'tpebl'),
    'category' => 'tpae',
    'input_schema' => [
        'type' => 'object',
        'properties' => [
            ...
            'prevText' => [
                'type' => 'string',
                'description' => 'Previous post link label',
            ],
            'nextText' => [
                'type' => 'string',
                'description' => 'Next post link label',
            ],
            'showcsttexonomy' => [
                'type' => 'string',
                'enum' => ['yes', 'no'],
                'description' => 'Restrict navigation to same category',
            ],
    ...
    'execute_callback' => 'tpae_mcp_add_theplus_post_navigation_ability',
    'permission_callback' => 'tpae_mcp_add_theplus_post_navigation_permission',
    'meta' => [
        'show_in_rest' => true,
 ...
 */
function tpae_mcp_add_theplus_post_navigation_permission(?array $input = null): bool
{
...
/**
 * Add a TP Post Navigation widget into an Elementor container.
 *
 * @param array<string, mixed> $input
 * @return array<string, mixed>|WP_Error
 */
function tpae_mcp_add_theplus_post_navigation_ability(array $input)
{
    if (!tpae_mcp_has_elementor()) {
        return new WP_Error('elementor_missing', __('Elementor must be active.', 'tpebl'));
    }

    $widget_type = 'tp-post-navigation';
    if (!tpae_mcp_has_registered_widget($widget_type)) {
        return new WP_Error('widget_missing', __('The Plus Post Navigation widget is not registered on this site.', 'tpebl'));
    }
    ...
    if (!empty($input['style'])) {
        $settings['style'] = sanitize_key((string) $input['style']);
    }

    if (!empty($input['prevText'])) {
        $settings['prevText'] = sanitize_text_field((string) $input['prevText']);
    }

    if (!empty($input['nextText'])) {
        $settings['nextText'] = sanitize_text_field((string) $input['nextText']);
    }

    if (!empty($input['showcsttexonomy'])) {
        $settings['showcsttexonomy'] = sanitize_key((string) $input['showcsttexonomy']);
    }
