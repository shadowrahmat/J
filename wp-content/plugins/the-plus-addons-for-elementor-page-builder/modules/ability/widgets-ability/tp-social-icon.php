...
wp_register_ability('tpae/tpae-social-icon', [
    'label' => __('Social Icon', 'tpebl'),
    'description' => __('Adds the The Plus "Social Icon" widget (tp-social-icon) to an Elementor container.', 'tpebl'),
    'category' => 'tpae',
    'input_schema' => [
        'type' => 'object',
        'properties' => [
            ...
            'styles' => [
                'type' => 'string',
                'enum' => ['style-1', 'style-2', 'style-3', 'style-4', 'style-5', 'style-6', 'style-7', 'style-8', 'style-9', 'style-10'],
                'description' => 'Icon button visual style',
            ],
            'social_icons_list' => [
                'type' => 'array',
                ...
                'items' => [
                    ...
                    'properties' => [
                        'pt_plus_social_icons' => [
                            'type' => 'string',
                            'enum' => ['facebook', 'twitter', 'instagram', 'linkedin', 'youtube', 'pinterest', 'whatsapp', 'github', 'dribbble', 'behance'],
    ...
    'execute_callback' => 'tpae_mcp_add_theplus_social_icon_ability',
    'permission_callback' => 'tpae_mcp_add_theplus_social_icon_permission',
    'meta' => [
        'show_in_rest' => true,
...
function tpae_mcp_add_theplus_social_icon_permission(?array $input = null): bool
{
...
function tpae_mcp_add_theplus_social_icon_ability(array $input)
{
    ...
    if (!tpae_mcp_has_registered_widget($widget_type)) {
        return new WP_Error('widget_missing', __('The Plus Social Icon widget is not registered.', 'tpebl'));
    }
    ...
    if (!empty($input['styles'])) { $settings['styles'] = sanitize_key((string) $input['styles']); }
    if (!empty($input['social_icons_list']) && is_array($input['social_icons_list'])) {
        $settings['social_icons_list'] = array_map(function ($icon) {
            return [
                'pt_plus_social_icons' => sanitize_key((string) ($icon['pt_plus_social_icons'] ?? 'facebook')),
                'social_url'           => ['url' => esc_url_raw((string) ($icon['social_url'] ?? ''))],
                '_id'                  => substr(bin2hex(random_bytes(4)), 0, 7),
            ];
        }, $input['social_icons_list']);
    }
