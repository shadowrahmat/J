...
wp_register_ability('tpae/tpae-style-list', [
    'label' => __('Style List', 'tpebl'),
    'description' => __('Adds the The Plus "Style List" widget (tp-style-list) to an Elementor container.', 'tpebl'),
    'category' => 'tpae',
    'input_schema' => [
        'type' => 'object',
        'properties' => [
            ...
            'list_items' => [
                ...
                'items' => [
                    ...
                    'properties' => [
                        ...
                        'icon_style' => [
                            'type' => 'string',
                            'enum' => ['None', 'font_awesome', 'font_awesome_5'],
                            'description' => 'Icon type for list item',
                        ],
                        'link' => [
                            'type' => 'string',
                            'description' => 'Optional link URL for item',
                        ],
                    ],
                ],
            ],
    ...
    'execute_callback' => 'tpae_mcp_add_theplus_style_list_ability',
    'permission_callback' => 'tpae_mcp_add_theplus_style_list_permission',
    'meta' => [
        'show_in_rest' => true,
...
function tpae_mcp_add_theplus_style_list_permission(?array $input = null): bool
{
...
function tpae_mcp_add_theplus_style_list_ability(array $input)
{
    ...
    if (!tpae_mcp_has_registered_widget($widget_type)) {
        return new WP_Error('widget_missing', __('The Plus Style List widget is not registered.', 'tpebl'));
    }
    ...
    if (!empty($input['list_items']) && is_array($input['list_items'])) {
        $settings['list_items'] = array_map(function ($item) {
            return [
                'content_description' => wp_kses_post((string) ($item['content_description'] ?? '')),
                'icon_style'          => sanitize_key((string) ($item['icon_style'] ?? 'None')),
                'link'                => ['url' => esc_url_raw((string) ($item['link'] ?? ''))],
