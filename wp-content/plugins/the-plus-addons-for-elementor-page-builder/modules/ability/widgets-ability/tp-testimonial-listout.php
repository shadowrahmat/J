...
wp_register_ability('tpae/tpae-testimonial-listout', [
    'label' => __('Testimonial', 'tpebl'),
    'description' => __('Adds the The Plus "Testimonial" widget (tp-testimonial-listout) to an Elementor container.', 'tpebl'),
    'category' => 'tpae',
    'input_schema' => [
        'type' => 'object',
        'properties' => [
            ...
            'style' => [
                ...
                'description' => 'Testimonial card visual style',
            ],
            'layout' => [
                'type' => 'string',
                'enum' => ['grid', 'masonry', 'carousel'],
                'description' => 'Testimonial listing layout',
            ],
    ...
    'execute_callback' => 'tpae_mcp_add_theplus_testimonial_listout_ability',
    'permission_callback' => 'tpae_mcp_add_theplus_testimonial_listout_permission',
    'meta' => [
        'show_in_rest' => true,
...
function tpae_mcp_add_theplus_testimonial_listout_permission(?array $input = null): bool
{
...
function tpae_mcp_add_theplus_testimonial_listout_ability(array $input)
{
    if (!tpae_mcp_has_elementor()) {
        return new WP_Error('elementor_missing', __('Elementor must be active.', 'tpebl'));
    }

    $widget_type = 'tp-testimonial-listout';
    if (!tpae_mcp_has_registered_widget($widget_type)) {
        return new WP_Error('widget_missing', __('The Plus Testimonial widget is not registered.', 'tpebl'));
    }
    ...
    if (!empty($input['layout'])) { $settings['layout'] = sanitize_key((string) $input['layout']); }
